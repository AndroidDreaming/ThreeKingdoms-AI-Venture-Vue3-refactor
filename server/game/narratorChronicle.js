...
async function streamFromProvider(settings, prompt, onText) {
  ...
  const requestBody = JSON.stringify({
    ...
    messages: [
      {
        role: 'system',
        content: '你负责把结构化剧情状态写成沉浸、克制、持续推进的汉末中文叙事，只输出正文，不要解释系统。'
      },
      {
        role: 'user',
        content: prompt
      ...
      while (true) {
        ...
        if (!checkedFirstChunk) {
          ...
          if (isHtmlResponse(response, chunkText)) {
            throw new Error('上游返回了页面内容而不是叙事流');
          }
        }
...
async function narrateScene(options) {
  ...
  if (onStatus) {
    await onStatus(providerEnabled ? '风声已起，这一页正在续写。' : '案头旧卷先替你续上这一回。');
  }
  ...
  try {
    ...
    if (onStatus) {
      await onStatus('外间文书迟迟未至，先由旧卷接住这一回。');
    }
  }
