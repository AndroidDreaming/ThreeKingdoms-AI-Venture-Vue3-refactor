...
function dynamicSlotRoleSummary(role) {
  return {
    normal: '这一格只负责最顺势、最容易理解、最能把当前局面往前推进的一手。',
    moral: '这一格必须让玩家看见具体的人情、公义、承诺、名声或安全代价。',
    wild: '这一格必须明显偏出正路，怪、险、戏剧化，但仍要贴着当前局势成立。'
  ...
  return [
    {
      ...
      intent: 'normal',
      label: '动作1',
      summary: '先给玩家一手最顺势、最容易理解、最能把当前局面往前推进的动作。',
    ...
    },
    {
      ...
      intent: 'moral',
      label: '动作2',
      summary: `把局往前推，但必须让某种关系、承诺、名声或公义真的出血。${expertTheme.summary}`,
    ...
    },
    {
      ...
      label: '动作3',
      summary: highRisk
        ? '场上冒出一道危险但值钱的偏门，这一格就该给出能抢人、截手、偷步或翻盘的怪招。'
        : '别只盯正面。这一格必须给出一手绕开明面、足够古怪但仍说得通的动作。',
...
function buildChoicePrompt(session, action) {
  ...
  return [
    ...
    '必须按固定顺序输出三张牌：1. normal 2. moral 3. wild。',
    '第一张：最顺势、最容易理解、最能把当前局势往前推的一手。',
    '第二张：必须明确让玩家看见具体的牺牲、利用、出卖、连累、欺骗，或者压掉情义、名声、公义、承诺中的至少一项。不能只写成“有风险”。',
    '第三张：必须明显偏离常规路径，看起来怪、险、戏剧化，甚至带一点黑色幽默，但仍然贴着当前局势成立。',
    ...
    '第二张与第三张的 actionText 或 hint 里，必须让玩家看见它为什么痛、为什么怪。',
...
function buildNarrationChoicesPrompt(session, action, narrationPrompt) {
  ...
  return [
    ...
    '必须按固定顺序输出三张牌：1. normal 2. moral 3. wild。',
    '第二张必须写出具体的人情、公义、名声、承诺或安全代价。',
    '第三张必须明显偏出常规路径，怪、险、戏剧化，但仍然成立。',
...
function buildDirectorTurnPrompt(directorPacket) {
  ...
  return [
    ...
    '不要生成“河北豪族、地方豪族、地方豪右、地方势力、部族、士族、宗族、望族、乡豪、强宗”等抽象势力目标，也不要写“把某物给某某豪族/地方势力”“拜会某地豪族”“借某股势力”“联络某部族”这类模板句。',
    '动态选项优先落在当场人物、刚发生的余波、可见地点、手头困境和玩家刚选择的行动上；目标必须具体到官署、军营、店铺、驿站、门吏、差役、兵卒、文书、榜文或当场人物。',
    '这三张 nextChoices 按顺序分别是：正常推进局势、带明显道德两难、反常理但有趣。',
    '第二张必须明确要求牺牲、利用、出卖、连累、欺骗某个人或某一群人，或者为了效率与收益去压掉情义、名声、公义、承诺中的至少一项。代价必须具体，不能只写成“有风险”。',
    '第三张必须明显偏离常规路径，让人一眼觉得怪、险、戏剧化，甚至有点好笑，但仍然能在当前情境里成立。它不能只是另一张调查牌、等待牌、试探牌或常规会面牌。',
    '如果第二张牌没有明确的人情、公义、名声或承诺代价，它就失败。',
    '如果第三张牌只是更激进一点的常规推进，或者只是换个对象继续查、继续等、继续谈，它就失败。',
    '把决定权交给剧情语境本身，不要假设本地已经替你规划好路线。text 足够时就不要硬补字段；actionText、hint 等字段只在自然需要时补充。',
    '第二张与第三张的 actionText 或 hint 里，必须让玩家看见它为什么痛、为什么怪，不能只写成普通行动说明。',
