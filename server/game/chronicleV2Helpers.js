...
function upsertRelationship(relationships, relationship) {
  ...
  const index = list.findIndex((item) => item.id === relationship.id || item.name === relationship.name);
  if (index === -1) {
    const created = Object.assign({ trust: 0, affection: 0 }, relationship);
    delete created.trustDelta;
    delete created.affectionDelta;
    list.push(created);
    return list;
  }

  const current = list[index];
  const next = Object.assign({}, current, relationship, {
    trust: clamp((current.trust || 0) + (relationship.trustDelta || 0), 0, 100),
    affection: clamp((current.affection || 0) + (relationship.affection || 0) + (relationship.affectionDelta || 0), 0, 100)
  });
  delete next.trustDelta;
  delete next.affectionDelta;
  list[index] = next;
  return list;
}
