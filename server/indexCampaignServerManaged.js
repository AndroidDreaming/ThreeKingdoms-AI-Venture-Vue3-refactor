...
const { classifyIntent } = require('./game/chronicleV3IntentClassifier');
const { narrateScene } = require('./game/narratorChronicle');
const { processAction } = require('./game/chronicleV3RulesEngine');
const { createSession, loadSession, resetSession, saveSession } = require('./game/chronicleV3SessionStore');
...
async function handleTurnStream(res, sessionId, body) {
  ...
  beginNdjson(res);
  writeStreamChunk(res, { type: 'status', message: '你已落子，局势正在起风。' });
...
if (require.main === module) {
  server.listen(PORT, () => {
    console.log(`ThreeKingdoms campaign backend running at http://localhost:${PORT}`);
  });
}
